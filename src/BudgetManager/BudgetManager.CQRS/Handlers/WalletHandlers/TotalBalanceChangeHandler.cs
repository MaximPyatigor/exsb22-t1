...
{
    public class TotalBalanceChangeHandler : IRequestHandler<ChangeTotalBalanceOfWalletCommand, bool>
    {
        private readonly IBaseRepository<User> _userRepository;

        public TotalBalanceChangeHandler(IBaseRepository<User> userRepository)
        {
            _userRepository = userRepository;
        }

        public async Task<bool> Handle(ChangeTotalBalanceOfWalletCommand request, CancellationToken cancellationToken)
        {
            var transactionObject = request.transaction;
            var filterDefinition = Builders<User>.Filter.Eq(u => u.Id, transactionObject.UserId)
            ...
            var user = (await _userRepository.FilterBy<User>(filterDefinition, projectionDefinition, cancellationToken)).FirstOrDefault();
            ...
            switch (transactionObject.TransactionType)
            {
                case OperationType.Income:
                    wallet.Balance += transactionObject.Value;
                    break;
